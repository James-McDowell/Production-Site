...
# def layout():
#     latest_posts = Post.query.order_by(Post.date_posted.desc()).get(2)
#     return redirect(url_for('main.home'), latest_posts=latest_posts)
...
def user_posts(username):
    ...
    return render_template('user_posts.html', posts=posts, user=user, cat=cat, featured=featured_posts,
                           projects=projects, next_page=next_page, prev_page=prev_page)
...
def user_profile(username):
    ...
    likes = PostLike.query.filter_by(liked_id=user.id).count()
    return render_template('user_profile.html', user=user, post=post, likes=likes)
